    ...
    <SafeAreaView className="flex-1 bg-white dark:bg-zinc-950">
      <View style={{ flex: 1, paddingTop: 24 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 32, marginBottom: 24 }}>
          <Pressable 
            onPress={() => router.back()}
            style={{ width: 44, height: 44, borderRadius: 16, backgroundColor: isDark ? '#18181B' : '#F0F9FF', alignItems: 'center', justifyContent: 'center', marginRight: 16, overflow: 'hidden' }}
          >
        ...
        >
          <Animated.View entering={FadeInDown.delay(100).duration(800).springify()} style={{ gap: 16 }}>
            {/* Identity Card */}
            <View className="rounded-[32px] overflow-hidden shadow-xl shadow-sky-200/30 dark:shadow-none bg-white dark:bg-zinc-900">
              <LinearGradient 
                colors={isDark ? ['#18181B', '#09090B'] : ['#FFFFFF', '#F0F9FF']}
                className="p-6 border border-white dark:border-white/10 flex-row items-center"
              >
            ...
            <View className="rounded-[32px] overflow-hidden shadow-lg shadow-sky-200/20 dark:shadow-none bg-white dark:bg-zinc-900">
              <LinearGradient 
                colors={isDark ? ['#18181B', '#09090B'] : ['#FFFFFF', '#F8FAFC']}
                className="p-8 border border-white dark:border-white/10"
              >
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                  {profile.interests.map((interest, idx) => (
                    <View key={idx} className="bg-sky-50 dark:bg-zinc-800/80 px-5 py-3 rounded-[20px] border border-sky-100 dark:border-zinc-700/50 shadow-sm overflow-hidden">
            ...
            <View style={{ gap: 16, marginTop: 12 }}>
              <TouchableOpacity 
                ...
                activeOpacity={0.8}
                className="w-full py-5 rounded-[28px] bg-white border border-sky-100 dark:bg-zinc-900 dark:border-zinc-800 shadow-xl shadow-sky-200/40 dark:shadow-none flex-row items-center justify-center overflow-hidden"
              >
              ...
              <TouchableOpacity 
                ...
                activeOpacity={0.6}
                className="w-full py-5 rounded-[28px] border border-rose-100 dark:border-rose-900/30 bg-rose-50/50 dark:bg-rose-950/20 active:scale-95 transition-transform duration-200 flex-row items-center justify-center overflow-hidden"
              >
