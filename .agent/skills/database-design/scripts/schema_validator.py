...
def find_schema_files(project_path: Path) -> list:
    ...
    for root, dirs, files in os.walk(project_path):
        dirs[:] = [d for d in dirs if d not in {'.git', 'node_modules', '.expo', 'android', 'ios', '.next', 'dist'}]
        root_path = Path(root)
        
        for file in files:
            if file == 'schema.prisma' and 'prisma' in root_path.parts:
                schemas.append(('prisma', root_path / file))
            elif file.endswith('.ts') and ('drizzle' in root_path.parts or 'schema' in file.lower() or 'table' in file.lower()):
                schemas.append(('drizzle', root_path / file))
